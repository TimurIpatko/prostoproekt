...
для всех возрастов результат "неудовлетворительная" отстоит от "слабого" на 4,
тот от "удовлетворительного" на 5, а "хороший" от "удовлетворительная" - на 5.5
...
def ruffier_result(r_index, level):
    ...
    if r_index >= level:
        return 0
    level = level - 4 # это не будет выполняться, если мы уже вернули ответ "неудовлетворительная"
    if r_index >= level:
        return 1
    level = level - 5 # аналогично, попадаем сюда, если уровень как минимум "удовлетворительная"
    if r_index >= level:
        return 2
